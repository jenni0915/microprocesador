...
static bool volatile  gb_swm_1s_evnt;     // timeout 1s
...
static void swm_to_1s_isr(void){
  gb_swm_1s_evnt = true;
  gb_swm_can_sleep = false;
  ...
  if (gb_swm_initd) {  // protect against calling swm_fsm() w/o a previous call to swm_init()
    ...
    switch (g_swm_state) {
      ...
      case SW_IN:
        ...
        gb_swm_1s_evnt = false;
        if(gb_swm_to_evnt){
          gb_swm_to_evnt = false;
          if( gb_swm_fall_evnt == *gp_swm){
            g_swm_to1s.attach(swm_to_1s_isr,1s);
            g_swm_state = SW_OUT;
          }else {
            g_swm_state = SW_IDLE;
          }
