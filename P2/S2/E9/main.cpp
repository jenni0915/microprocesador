...
static Ticker g_lit_tick;
static Ticker g_tick;
static bool volatile gb_toggle;
static bool volatile gb_lit_toggle;
...
int main (void) {
  ...
  g_dsr.pulsewidth(lit);
  g_dsl.pulsewidth(0);

  g_lit_tick.attach(tick_lit_isr,333ms);
  g_tick.attach(tick_isr,4ms);
  g_dsr = b_right;
  g_dsl = !b_right;
      g_seg = to_7seg(b_right ? contador % 10 : contador / 10);

  lit = g_lit.read_u16()*100/65535;
  ...
  for (;;) {
    ...
    if( gb_toggle ){
      ...
      g_dsr = b_right;
      g_dsl = !b_right;
      g_dsl.pulsewidth_us((b_right) ? 0 : lit);
      g_dsr.pulsewidth_us((b_right) ? lit : 0);
